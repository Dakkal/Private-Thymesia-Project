...
class CEnemy_Shield final : public CLandObject
{
protected:
	CEnemy_Shield(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	CEnemy_Shield(const CEnemy_Shield& rhs);

	virtual ~CEnemy_Shield() = default;
...
public:
	static CEnemy_Shield* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, const wstring& strProtoTag = TEXT(""));
