...
class CHitBox_Shield final : public CPartObject
{
protected:
	CHitBox_Shield(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	CHitBox_Shield(const CHitBox_Shield& rhs);
	virtual ~CHitBox_Shield() = default;
...
public:
	static CHitBox_Shield* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, const wstring& strProtoTag = TEXT(""));
