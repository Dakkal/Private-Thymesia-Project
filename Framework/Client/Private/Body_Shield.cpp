...
CBody_Shield::CBody_Shield(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CPartObject(pDevice, pContext)
{

}

CBody_Shield::CBody_Shield(const CBody_Shield& rhs)
	: CPartObject(rhs)
{

}

HRESULT CBody_Shield::Initialize_Prototype(const wstring& strProtoTag)
{
	__super::Initialize_Prototype(strProtoTag);

	m_eObjType = OBJECT_TYPE::PART;
	m_strObjectName = TEXT("Enemy_Shield_Body");

	return S_OK;
}

HRESULT CBody_Shield::Initialize(void* pArg)
{
...
void CBody_Shield::Tick(_float fTimeDelta)
{
...
void CBody_Shield::LateTick(_float fTimeDelta)
{
	if (false == g_EditMode)
...
HRESULT CBody_Shield::Render_LightDepth()
{
...
void CBody_Shield::OnCollision_Enter(CGameObject* _pColObj, _float fTimeDelta)
{
...
void CBody_Shield::OnCollision_Stay(CGameObject* _pColObj, _float fTimeDelta)
{
	OBJECT_TYPE eObject = _pColObj->Get_ObjectType();
...
void CBody_Shield::OnCollision_Exit(CGameObject* _pColObj, _float fTimeDelta)
{
	OBJECT_TYPE eObject = _pColObj->Get_ObjectType();
...
void CBody_Shield::OnCollision_Part_Enter(CGameObject* _pColObj, _float fTimeDelta)
{
	CGameInstance* pGameInstance = GET_INSTANCE(CGameInstance);
...
void CBody_Shield::OnCollision_Part_Stay(CGameObject* _pColObj, _float fTimeDelta)
{
	CGameInstance* pGameInstance = GET_INSTANCE(CGameInstance);
...
void CBody_Shield::OnCollision_Part_Exit(CGameObject* _pColObj, _float fTimeDelta)
{
	CGameObject* pPartOwner = dynamic_cast<CPartObject*>(_pColObj)->Get_PartOwner();
	OBJECT_TYPE eOwnerType = pPartOwner->Get_ObjectType();
	CGameObject::PARTS ePart = dynamic_cast<CPartObject*>(_pColObj)->Get_Part_Index();
...
HRESULT CBody_Shield::Ready_Components()
{
	...
	if (true == g_EditMode)
	{
		...
		/* Com_Model */
		if (FAILED(__super::Add_Component(LEVEL_EDIT, TEXT("Prototype_Component_Model_Enemy_Shield_Body"),
	...
	else
	{
		...
		/* Com_Model */
		if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Model_Enemy_Shield_Body"),
			TEXT("Com_Model"), (CComponent**)&m_pModelCom)))
			return E_FAIL;
...
HRESULT CBody_Shield::Bind_ShaderResources()
{
	if (FAILED(m_pShaderCom->Bind_Matrix("g_WorldMatrix", &m_WorldMatrix)))
		return E_FAIL;
...
CBody_Shield* CBody_Shield::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, const wstring& strProtoTag)
{
	CBody_Shield* pInstance = new CBody_Shield(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype(strProtoTag)))
	{
		MSG_BOX("Failed to Created : CBody_Shield");
...
CGameObject* CBody_Shield::Clone(void* pArg)
{
	__super::Clone(pArg);

	CBody_Shield* pInstance = new CBody_Shield(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CBody_Shield");
		Safe_Release(pInstance);
	}
