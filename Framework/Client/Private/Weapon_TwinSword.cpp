...
CWeapon_TwinSword::CWeapon_TwinSword(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CPartObject(pDevice, pContext)
{

}

CWeapon_TwinSword::CWeapon_TwinSword(const CWeapon_TwinSword& rhs)
	: CPartObject(rhs)
{

}

HRESULT CWeapon_TwinSword::Initialize_Prototype(const wstring& strProtoTag)
{
	__super::Initialize_Prototype(strProtoTag);

	m_eObjType = OBJECT_TYPE::PART;
	m_strObjectName = TEXT("Enemy_TwinSword_Weapon");

	return S_OK;
}

HRESULT CWeapon_TwinSword::Initialize(void* pArg)
{
	...
	m_pTransformCom->Rotation(_vector(1.f, 0.f, 0.f, 0.f), XMConvertToRadians(120.f));
	m_pTransformCom->Rotation(_vector(0.f, 1.f, 0.f, 0.f), XMConvertToRadians(30.f));
	m_pTransformCom->Rotation(_vector(0.f, 0.f, 1.f, 0.f), XMConvertToRadians(-65.f));

	return S_OK;
}

void CWeapon_TwinSword::Tick(_float fTimeDelta)
{
...
void CWeapon_TwinSword::LateTick(_float fTimeDelta)
{
...
HRESULT CWeapon_TwinSword::Render()
{
...
HRESULT CWeapon_TwinSword::Render_LightDepth()
{
...
void CWeapon_TwinSword::OnCollision_Enter(CGameObject* _pColObj, _float fTimeDelta)
{
...
void CWeapon_TwinSword::OnCollision_Stay(CGameObject* _pColObj, _float fTimeDelta)
{
	OBJECT_TYPE eObject = _pColObj->Get_ObjectType();
...
void CWeapon_TwinSword::OnCollision_Exit(CGameObject* _pColObj, _float fTimeDelta)
{
	OBJECT_TYPE eObject = _pColObj->Get_ObjectType();
...
void CWeapon_TwinSword::OnCollision_Part_Enter(CGameObject* _pColObj, _float fTimeDelta)
{
...
void CWeapon_TwinSword::OnCollision_Part_Stay(CGameObject* _pColObj, _float fTimeDelta)
{
	CGameObject* pPartOwner = dynamic_cast<CPartObject*>(_pColObj)->Get_PartOwner();
	OBJECT_TYPE eOwnerType = pPartOwner->Get_ObjectType();
	CGameObject::PARTS ePart = dynamic_cast<CPartObject*>(_pColObj)->Get_Part_Index();
...
void CWeapon_TwinSword::OnCollision_Part_Exit(CGameObject* _pColObj, _float fTimeDelta)
{
	CGameObject* pPartOwner = dynamic_cast<CPartObject*>(_pColObj)->Get_PartOwner();
	OBJECT_TYPE eOwnerType = pPartOwner->Get_ObjectType();
	CGameObject::PARTS ePart = dynamic_cast<CPartObject*>(_pColObj)->Get_Part_Index();
...
HRESULT CWeapon_TwinSword::Ready_Components()
{
	/* Com_Renderer */
...
HRESULT CWeapon_TwinSword::Bind_ShaderResources()
{
	if (FAILED(m_pShaderCom->Bind_Matrix("g_WorldMatrix", &m_WorldMatrix)))
		return E_FAIL;
...
CWeapon_TwinSword* CWeapon_TwinSword::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, const wstring& strProtoTag)
{
	CWeapon_TwinSword* pInstance = new CWeapon_TwinSword(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype(strProtoTag)))
	{
		MSG_BOX("Failed to Created : CWeapon_TwinSword");
...
CGameObject* CWeapon_TwinSword::Clone(void* pArg)
{
	__super::Clone(pArg);

	CWeapon_TwinSword* pInstance = new CWeapon_TwinSword(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CWeapon_TwinSword");
		Safe_Release(pInstance);
	}
