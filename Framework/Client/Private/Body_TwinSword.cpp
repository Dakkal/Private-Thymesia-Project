#include "pch.h"
#include "..\Public\Body_TwinSword.h"
...
CBody_TwinSword::CBody_TwinSword(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CPartObject(pDevice, pContext)
{

}

CBody_TwinSword::CBody_TwinSword(const CBody_TwinSword& rhs)
	: CPartObject(rhs)
{

}

HRESULT CBody_TwinSword::Initialize_Prototype(const wstring& strProtoTag)
{
	__super::Initialize_Prototype(strProtoTag);

	m_eObjType = OBJECT_TYPE::PART;
	m_strObjectName = TEXT("Enemy_TwinSword_Body");

	return S_OK;
}

HRESULT CBody_TwinSword::Initialize(void* pArg)
{
...
void CBody_TwinSword::Tick(_float fTimeDelta)
{
...
void CBody_TwinSword::LateTick(_float fTimeDelta)
{
	if (false == g_EditMode)
...
HRESULT CBody_TwinSword::Render()
{
...
HRESULT CBody_TwinSword::Render_LightDepth()
{
...
void CBody_TwinSword::OnCollision_Enter(CGameObject* _pColObj, _float fTimeDelta)
{
...
void CBody_TwinSword::OnCollision_Stay(CGameObject* _pColObj, _float fTimeDelta)
{
	OBJECT_TYPE eObject = _pColObj->Get_ObjectType();
...
void CBody_TwinSword::OnCollision_Exit(CGameObject* _pColObj, _float fTimeDelta)
{
	OBJECT_TYPE eObject = _pColObj->Get_ObjectType();
...
void CBody_TwinSword::OnCollision_Part_Enter(CGameObject* _pColObj, _float fTimeDelta)
{
	CGameInstance* pGameInstance = GET_INSTANCE(CGameInstance);
...
void CBody_TwinSword::OnCollision_Part_Stay(CGameObject* _pColObj, _float fTimeDelta)
{
	CGameInstance* pGameInstance = GET_INSTANCE(CGameInstance);
...
void CBody_TwinSword::OnCollision_Part_Exit(CGameObject* _pColObj, _float fTimeDelta)
{
	CGameObject* pPartOwner = dynamic_cast<CPartObject*>(_pColObj)->Get_PartOwner();
	OBJECT_TYPE eOwnerType = pPartOwner->Get_ObjectType();
	CGameObject::PARTS ePart = dynamic_cast<CPartObject*>(_pColObj)->Get_Part_Index();
...
HRESULT CBody_TwinSword::Ready_Components()
{
	...
	if (true == g_EditMode)
	{
		...
		/* Com_Model */
		if (FAILED(__super::Add_Component(LEVEL_EDIT, TEXT("Prototype_Component_Model_Enemy_TwinSword_Body"),
	...
	else
	{
		...
		/* Com_Model */
		if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Model_Enemy_TwinSword_Body"),
			TEXT("Com_Model"), (CComponent**)&m_pModelCom)))
			return E_FAIL;
...
HRESULT CBody_TwinSword::Bind_ShaderResources()
{
	if (FAILED(m_pShaderCom->Bind_Matrix("g_WorldMatrix", &m_WorldMatrix)))
		return E_FAIL;
...
CBody_TwinSword* CBody_TwinSword::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, const wstring& strProtoTag)
{
	CBody_TwinSword* pInstance = new CBody_TwinSword(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype(strProtoTag)))
	{
		MSG_BOX("Failed to Created : CBody_TwinSword");
...
CGameObject* CBody_TwinSword::Clone(void* pArg)
{
	__super::Clone(pArg);

	CBody_TwinSword* pInstance = new CBody_TwinSword(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CBody_TwinSword");
		Safe_Release(pInstance);
	}
