...
_bool CBounding_AABB::IsCollision(CCollider::TYPE eType, CBounding* pBouding)
{
	...
	case CCollider::TYPE_AABB:
		m_IsColl = m_pAABB->Intersects(*(dynamic_cast<CBounding_AABB*>(pBouding))->Get_Bouding());
		break;
	case CCollider::TYPE_OBB:
		m_IsColl = m_pAABB->Intersects(*(dynamic_cast<CBounding_OBB*>(pBouding))->Get_Bouding());
		break;
	case CCollider::TYPE_SPHERE:
		m_IsColl = m_pAABB->Intersects(*(dynamic_cast<CBounding_Sphere*>(pBouding))->Get_Bouding());
		break;
	}
